...
function PublicServicesComponent({ query }) {
    ...
    return (
        <>
            ...
            <Accordion className="container w-75 my-4 mb-5" dir="rtl">
                {filteredServices.map((element, index) => (
                    <Accordion.Item eventKey={index.toString()} key={element.id}>
                        ...
                        <Accordion.Body dir="rtl">
                            ...
                            <Link to={`/public/${element.id}`} className="btn btn-outline-primary mt-2 fs-5">
                                الذهاب إلى الخدمة
                            </Link>
                        </Accordion.Body>
                    </Accordion.Item>
